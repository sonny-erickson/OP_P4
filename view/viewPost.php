...
<div class="container text-light ">
	<h4 class="text-warning border-top border-bottom mt-5"><strong><?= htmlspecialchars($post['title']); ?></strong>le <?= htmlspecialchars($post['date_created']); ?></h4>
	<img src="assets/images/articleimg.jpg" alt="articleimg">
	<p class="text-light text-justify mt-4"> <?= ($post['content']); ?>
	...
	<div class="jumbotron jumbotron-fluid text-dark p-2 mb-2">
		<h5 class="border-bottom"><?= htmlspecialchars($comment['pseudo']);?> le <?= htmlspecialchars($comment['date_comment']);?><a class="btn btn-outline-danger btn-sm ml-4" href="index.php?page=signale&amp;id=<?= $comment['id']?>&amp;postId=<?= $post['id'] ?>" >Signaler</a></h5>
